...
| [openai-responses-compact](demos/openai-responses-compact/) | History compaction for the OpenAI Responses API. Turn 1 is stored in SQLite; on the follow-up Praxis rehydrates the history and, once it crosses `compact_threshold`, summarizes prior turns via an inference callout before forwarding. Also shows the explicit `POST /v1/responses/compact` endpoint returning a `response.compaction` object. |
